...
const ListResource = () => {
    ...
    const fetchCourse = async () => {
        try {
            ...
            setProv(tokendata.id);
            // console.log(tokendata);
            console.log(tokendata.id);
            const result = await Axios.get('http://localhost:5000/api/resources/' + tokendata.id);
            console.log(result.data);
            setCourses(result.data.myresources);
        } catch (err) {
            console.log(err);
        }
    }
    ...
    return (
        ...
        // < AddCourse add_course_page_disable={setState}/>
        <div className='w-full h-full overflow-scroll bg-gray-50'>
            {state===true ? <Addresources fun={setState} prov={prov}/> : <div className='flex-col h-[85%] w-full items-center'>
                <div style={{display:"flex", justifyContent:"space-between"}} className='w-full items-center pt-4'>
                    <h2 className='font-semibold text-md ml-24 mb-0'>Uploaded Resources</h2>
                    <button
                ...
                {/* <div className='bg-white h-full w-[90%] m-auto mt-5 rounded-lg flex-col'> */}
                    {courses.length <= 0 ? <div className='text-3xl mt-16'>No Resources added Yet</div> : courses.map(course => {
                        return (
